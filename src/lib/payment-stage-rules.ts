...
/**
 * Payment schedule rules — client-safe.
 * Validation, defaults, status helpers. No Prisma.
 */
...
export function defaultPaymentStagesFromProposal(proposal: {
  ...
  const total = roundMoney(Number(proposal.price) || 0)
  ...
  const deposit = roundMoney(Math.min(depositFor(proposal), total))
  const remainder = roundMoney(Math.max(0, total - deposit))
...
export function validatePaymentStages(
  ...
  const expected = roundMoney(Number(quoteTotal) || 0)
  ...
  const sum = roundMoney(stages.reduce((s, st) => s + st.amount, 0))
  const diff = roundMoney(sum - expected)
 ...
 * First stage: open once the contract is signed.
